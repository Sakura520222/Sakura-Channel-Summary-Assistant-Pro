...
### 🔧 技术优势
- **配置热重载**：支持配置文件实时监控和自动重载，修改配置无需重启
...
系统支持配置文件的实时监控和自动重载，修改配置后无需重启机器人即可生效。

**支持的配置类型**：

| 配置文件 | 配置项 | 热重载支持 | 说明 |
|---------|---------|-----------|------|
| `.env` | `LOG_LEVEL` | ✅ 支持 | 日志级别可动态更新 |
| `data/config/config.json` | 所有配置 | ✅ 支持 | 包含频道列表、时间配置、投票配置等 |
| `data/config/prompt.txt` | 总结提示词 | ✅ 支持 | AI总结提示词 |
| `data/config/poll_prompt.txt` | 投票提示词 | ✅ 支持 | 投票生成提示词 |

**自动模式（推荐）**：

安装 `watchdog` 库后，系统会自动监控配置文件变化：
...
- 自动验证新配置的有效性
- 原子化更新，配置验证失败则回滚
- 修改频道配置或时间配置会自动重启调度器（后台异步执行）

**启动时状态**：
- ✅ 已安装 watchdog：自动监控启用，配置文件变化后2秒自动重载
- ⚠️ 未安装 watchdog：自动监控禁用，需使用 `/reload` 命令手动重载

**手动重载**：

即使未安装 watchdog，也可以通过命令手动重载配置：
...
详细结果：
• env: 成功 - 环境变量已重载，日志级别已更新
• config: 成功 - JSON配置已重载
  - 频道数: 3
  - 总结配置: 3
  - 投票配置: 2
  - 调度器已重启
• prompt: 成功 - 总结提示词已更新（长度: 256）
• poll_prompt: 成功 - 投票提示词已更新（长度: 189）
```

**注意事项**：

1. **AI配置限制**：
   - AI API Key、Base URL 等只能通过 `.env` 文件配置
   - 修改 AI 配置后需要重启机器人才能生效

2. **调度器重启**：
   - 修改频道列表（`channels`）或总结时间配置（`summary_schedules`）会触发调度器重启
   - 调度器在后台异步重启，不会阻塞配置重载流程
   - 重启时会等待运行中的任务完成，确保数据一致性

3. **配置验证**：
   - 所有配置重载前都会验证格式和内容
   - 验证失败时，旧配置继续生效，系统记录错误日志
   - 提示词不能为空，否则重载会失败

4. **Docker环境**：
   - Docker 容器支持完整的自动监控功能
   - 修改宿主机的配置文件后，容器内会自动检测变化
   - 无需重启容器
...
```bash
# 查看配置监控器日志
docker-compose logs | grep "配置热重载"
```

日志示例：
```
INFO: 配置文件监控器已启动
INFO: 检测到配置文件变化: data/config/config.json
INFO: 开始重载配置文件: data/config/config.json
INFO: JSON配置重载成功: {'channels': 3, 'summary_schedules': 3, ...}
INFO: 调度器已重启
```
...
│   ├── config.py                  # 配置管理模块
│   ├── config_watcher.py          # 配置文件监控模块（热重载）
│   ├── config_reloader.py         # 配置重载管理模块
│   ├── database.py                # 数据库管理模块
│   ├── scheduler.py               # 调度器模块
