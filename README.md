...
| 命令 | 别名 | 功能说明 |
|------|------|----------|
| `/showchannels` | `/查看频道列表` | 查看所有配置的频道 |
| `/addchannel` | `/添加频道` | 添加新的频道到监控列表 |
...
自动防护机制：
- 系统自动检测恶意用户多次拉入机器人
- 在配置的时间窗口内达到违规阈值，自动加入黑名单
- 黑名单用户将被立即拒绝，机器人自动退出

手动管理：
- 管理员可以手动添加/移除黑名单
- 查看完整的黑名单列表和统计信息
- 防止误将管理员添加到黑名单（双重权限保护）
...
# 1小时内违规3次自动加入黑名单
BLACKLIST_THRESHOLD_COUNT=3
BLACKLIST_THRESHOLD_HOURS=1
```
